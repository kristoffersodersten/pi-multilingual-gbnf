...
export function detectLanguage(text: string): LanguageCode {
  ...
  // Swedish markers
  if (/\b(skapad|skapar|läst|läser|raderad|uppdatera|fel|klar|väntar)\b/.test(lower)) {
    return 'sv';
  }
  
  // German markers
  if (/\b(erstellt|erstelle|gelesen|bearbeitet|gelöscht|fehler|fertig|ausstehend)\b/.test(lower) 
      || /\b(ä|ö|ü)\b/.test(lower)) {
    return 'de';
  }
  
  // Spanish markers
  if (/\b(creado|creando|leído|leyendo|editado|eliminado|error|hecho|pendiente)\b/.test(lower) 
      || /\b(á|é|í|ó|ú|ñ)\b/.test(lower)) {
    return 'es';
  }
  
  // French markers
  if (/\b(créé|crée|lu|lisant|édité|édite|supprimé|supprime|erreur|fait|en attente)\b/.test(lower) 
      || /\b(é|è|ê|à|ù)\b/.test(lower)) {
    return 'fr';
  }
  
  // Italian markers
  if (/\b(creato|crea|letto|leggendo|modificato|modifica|eliminato|elimina|errore|fatto|in attesa)\b/.test(lower)) {
    return 'it';
  }
  
  // Portuguese markers
  if (/\b(criado|cria|lido|lendo|editado|edita|excluído|exclua|erro|feito|pendente)\b/.test(lower) 
      || /\b(ã|õ|ç)\b/.test(lower)) {
    return 'pt';
  }
  
  // Dutch markers
  if (/\b(gemaakt|maak|gelezen|lezen|bewerkt|bewerk|verwijderd|verwijder|fout|klaar|in behandeling)\b/.test(lower)) {
    return 'nl';
  }
  
  // Polish markers
  if (/\b(utworzony|twórz|przeczytany|czytam|edytowany|edytuj|usunięty|usuń|błąd|gotowe|oczekiwanie)\b/.test(lower) 
      || /\b(ą|ę|ś|ć|ź|ż|ół|ń)\b/.test(lower)) {
    return 'pl';
  }
...
export function getSupportedLanguages(): LanguageCode[] {
  return ['en', 'sv', 'de', 'es', 'fr', 'it', 'pt', 'nl', 'pl'];
}
